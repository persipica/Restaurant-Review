...
import { kakaoLogin, loginMember } from '../../api/memberApi';
...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    const requestKakaoLogin = async () => {
      try {
        ...
        openModal(
          'error',
          '카카오 회원가입이 필요합니다. 회원가입 페이지로 이동합니다.',
  ...
  const handleClickLogin = async () => {
    if (!loginParam.email || !loginParam.password) {
      openModal('info', '이메일과 비밀번호를 입력해주세요.');
      return;
    }

    try {
      const data = await loginMember(loginParam);
      login(data);
      ...
      openModal('error', '이메일 또는 비밀번호를 확인해주세요.');
    }
  };
      ...
      <div className="mx-auto max-w-3xl px-4 py-12">
        <div className="border-2 border-black bg-white p-8 shadow-[6px_6px_0_0] shadow-black">
          ...
          <p className="mt-2 text-sm text-gray-500">
            TasteMap에 로그인하고 나만의 맛집 리뷰를 남겨보세요.
          </p>
